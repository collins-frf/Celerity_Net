...
def plot_for_gif2(img_mean, snap_mean, label_mean, pred_mean, diff_mean, i, pred_list, uncertainty_2d):
    ...
    ax4 = fig.add_subplot(grid[1, 1])
    ax5 = fig.add_subplot(grid[1, 2])
    print("making th egif")
    norm = mpl.cm.colors.Normalize(vmax=1, vmin=-6)
    cmap = mpl.cm.gist_earth

    img_cmap = mpl.cm.Greys_r
    imgnorm = ImageNormalize(img_mean, interval=ManualInterval(vmin=.19, vmax=1), stretch=SqrtStretch(), clip=True)
    snapnorm = ImageNormalize(snap_mean, interval=ManualInterval(vmin=.19, vmax=1), stretch=SqrtStretch(), clip=True)

    ax0.imshow(img_mean[:, :, i], cmap='Greys_r', norm=imgnorm,
                     extent=[zeroline, crosshore_distance_meters, 0, alongshore_distance_meters])
    ...
    ax0.clabel(cs, fmt=fmt,
                  inline_spacing=2, fontsize='x-small', )
    cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=imgnorm, cmap=img_cmap), ax=ax0)
    cbar.set_label('Pixel Intensity', fontsize=14)
    ...
    ax3.imshow(snap_mean[:, downsample_zeroline:, i], cmap='Greys_r', norm=snapnorm,
               extent=[zeroline, crosshore_distance_meters, 0, alongshore_distance_meters])
    ...
    ax3.clabel(cs, fmt=fmt,
               inline_spacing=2, fontsize='x-small', )
    cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=snapnorm, cmap=img_cmap), ax=ax3)
    cbar.set_label('Pixel Intensity', fontsize=14)
    ...
    image = np.frombuffer(fig.canvas.tostring_rgb(), dtype='uint8')
    image = image.reshape(fig.canvas.get_width_height()[::-1] + (3,))
    plt.savefig(("./results/plot/result1s" + str(i) + ".png"))
    plt.close('all')
    #return image
...
def plot_for_paper(img_mean, label_mean, pred_mean, diff_mean, i, pred_list):
    ...
    img_norm = ImageNormalize(img_mean, interval=ManualInterval(vmin=.19, vmax=1), stretch=SqrtStretch(), clip=True)
    img_cmap = mpl.cm.Greys_r

    ax0.imshow(img_mean[:, downsample_zeroline:, i], cmap='Greys_r', norm=img_norm,
                     extent=[zeroline, crosshore_distance_meters, 0, alongshore_distance_meters])
    #ax0 = fig.add_subplot(2, 3, 1), plt.plot([zeroline, crosshore_distance_meters], [258, 258], color='r')
    cs = ax0.contour(X, Y, np.where(label_up[:, :img_cols] > .1, 0, np.flip(label_up[:, :img_cols], axis=0)), vmin=-4, vmax=2, alpha=.5,
                        colors=['white', 'white', 'white', 'white', 'white', 'white', 'white', 'white', 'white',
    ...
    ax1.set_title('j) Predicted', fontsize=16)
    cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax1)
    cbar.set_label('Elevation (m)', fontsize=14)
    ...
    ax2.set_title('k) Difference', fontsize=16)
...
def plot(img_mean, snap_mean, label_mean, pred_mean, diff_mean, rms2d_mean, mae2d_mean,
                 uncertainty_2d, diff_histo, rms_histo, pred_list, wc_list, within_2d, within_2d_mean, mae1d,
         diff1d, within1d, U_bin, U_mae, H_bin, H_rms, H_bias, D_bin, D_rms, D_bias, F_bin, F_rms, F_bias, mae1d2,
    ...
    if args.fullstats:
        ...
        ax1.legend()
        imgnorm = ImageNormalize(img_mean, interval=ManualInterval(vmin=0, vmax=895), stretch=SqrtStretch(), clip=True)
        ...
        ax2.set_title("b) 2D Underwater Occurrence", fontsize=16)
        ax2.set_ylabel("Alongshore (m)", fontsize=12)
        ax2.set_xlabel("Cross-shore (m)", fontsize=12)
        cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=imgnorm, cmap='jet'), ax=ax2)
        cbar.set_label('Subaqueous Count', fontsize=14)
        ...
        ax1.set_ylabel('RMSE (m)', fontsize=16)
    ...
    if args.indiv_pred_show:
        ...
        while i < test_set_length:
            print("in ips")
            #try:
            plot_for_paper(img_mean, label_mean, pred_mean, diff_mean, i, pred_list)
            i += 95
            #plt.show()
            #except:
            #    continue
...
if __name__ == '__main__':
    ...
    mae1d = np.load('./results/plot/' + name + str(ensemble_runs) + "mae1d.npy")
    diff1d = np.load('./results/plot/' + name + str(ensemble_runs) + "diff1d.npy")
    within1d = np.load('./results/plot/' + name + str(ensemble_runs) + "within1d.npy")
    U_bin = np.load('./results/plot/' + name + str(ensemble_runs) + "U_bin.npy", allow_pickle=True)
    U_mae = np.load('./results/plot/' + name + str(ensemble_runs) + "U_mae.npy", allow_pickle=True)
    H_bin = np.load('./results/plot/' + name + str(ensemble_runs) + "H_bin.npy", allow_pickle=True)
    H_rms = np.load('./results/plot/' + name + str(ensemble_runs) + "H_rms.npy", allow_pickle=True)
    H_bias = np.load('./results/plot/' + name + str(ensemble_runs) + "H_bias.npy", allow_pickle=True)
    D_bin = np.load('./results/plot/' + name + str(ensemble_runs) + "D_bin.npy", allow_pickle=True)
    D_rms = np.load('./results/plot/' + name + str(ensemble_runs) + "D_rms.npy", allow_pickle=True)
    D_bias = np.load('./results/plot/' + name + str(ensemble_runs) + "D_bias.npy", allow_pickle=True)
    F_bin = np.load('./results/plot/' + name + str(ensemble_runs) + "F_bin.npy", allow_pickle=True)
    F_rms = np.load('./results/plot/' + name + str(ensemble_runs) + "F_rms.npy", allow_pickle=True)
    F_bias = np.load('./results/plot/' + name + str(ensemble_runs) + "F_bias.npy", allow_pickle=True)
